...
class exigencias{
    ...
    irParaEmpreendimento() {cy.get(el.abaLateralEmpreendimento).click()}
    irParaDemanda() {cy.get(el.abaDemanda).click()}
    digitarNumeroExigencia() {cy.get(el.coExigencia).type('34243')}
    clicarBotaoLimpar() {cy.get(el.botaoLimparExigencia).click()}
    selecionarTema() {cy.get(el.seletorTema).click()}
    digitarPrazoDias() {cy.get(el.campoPrazoDias).type('10')}
    digitarPrevCumprimentoEApagar(data){cy.get('#dt_previsao_cumprimento').type(data).clear()}
    ...
    selecionarTitular() {cy.get(el.seletorPessoaTitular).click()}
    selecionarSuplente() {cy.get(el.seletorSuplente).click()}
    selecionarStatus() {cy.get(el.seletorStatus).click()}
    ...
    clicarAbaDemanda() {cy.get(el.botaoDemanda).click()}

    clicarConsultarDemanda() {cy.get(el.botaoConsultaDemanda).click()}
    ...
    digitarFiltroPesquisarDemanda() {cy.get(el.filtroPesquisarDemanda).type('12345454')}
    ...
    selecionarRaPesquisarExigencias(){cy.get(el.seletorRegiao).click()}
    selecionarEmpreendimentoPesquisarExigencias(){
        cy.wait(5000)
        cy.get(el.seletorEmpreendimentoPesquisarExigencias).click()
    }
    selecionarDropdown(selecao){cy.get(el.dropdownGla).find('li').contains(selecao).click({force: true})}
    botaoPesquisarExigencia(){cy.get(el.botaoPesquisarExigencia).click()}
    digitarFiltroPesquisar(texto){cy.get(el.campoFiltroPesquisa).type(texto)}
    ...
    clicarBotaoNao(){cy.contains('button', "Não").click()}
    botaoEditarDilacaoPrazos(){cy.get(el.botaoEditarDilacaoPrazos).click()}
    botaoExcluir(){cy.get(el.botaoExcluir).click()}
    botaoModalSim(){cy.contains('button', "Sim").click()}
    clicarBotaoEditarExigencia(){cy.get(el.botaoLinkEditarExigencia).click()}
    digitarQuantidadePrazoDias(){cy.get(el.campoPrazoDias).type('80')}
    salvarBotao(){cy.get('#submit').click()}
    clicarModalSalvar(){cy.get(el.botaoModalSalvar).click()}
    clicarBotaoOK(){cy.contains('button', "OK").click()}
    digitarCoExigencia(texto){cy.get(el.labelCoExigencia).type(texto)}
    clicarBotaoExcluirExigencia(){cy.get(el.botaoDeleteExigencia).click()}
    clicarModalConfirmar(){cy.contains('button',"Confirmar").click()}
    clicarModalRelatorioPDF(){cy.get(el.modalRelatorioPDF).click()}
    clicarGerarRelatorioPDF(){cy.get(el.gerarRelatorioPDF).click()}
    fecharModalPDF(){cy.get(el.botaoFecharModalPDF).click()
    }

    //VALIDAR CAMPOS DE TEXTO
    validarLabelEmpreendimento(){cy.get(el.labelEmpreendimento).should('contain', "Empreendimento ")}
    validarLabelDemanda() {cy.get(el.labelDemanda).should('contain', 'Demanda ')}
    validarLabelNumero() {cy.get(el.labelNumero).should('contain', 'Número')}
    validarLabelTema() {cy.get(el.labelTema).should('contain', 'Tema')}
    validarLabelPossuiPrazoExpresso() {cy.get(el.labelPossuiPrazoExpresso).should('contain', 'Possui Prazo Expresso na Exigência')}
    validarLabelDias() {cy.get(el.labelDias).should('contain', 'Dias')}
    validarLabelPrevisaoCumprimento() {cy.get(el.labelPrevisaoCumprimento).should('contain', 'Previsão de Cumprimento')}
    validarLabelExigeContratacao() {cy.get(el.labelExigeContratacao).should('contain', 'Exige Contratação')}
    validarExigeContratacaoNoCheck(){cy.get('#fl_exige_contratacao').should('not.be.checked')}
    validarLabelDescricaoExigencia() {cy.get(el.labelDescricaoExigencia).should('contain', 'Descrição Exigência')}
    validarLabelDescricaoProvidencia() {cy.get(el.labelDescricaoProvidencia).should('contain', 'Descrição Providência')}
    ...
    validarLabelStatus() {cy.get(el.labelStatus).should('contain', 'Status')}
    validarLabelSeiProcAcompanhamento() {cy.get(el.labelSeiProcAcompanhamento).should('contain', 'SEI Proc. de Acompanhamento')}
    validarLabelUnidadeInterna() {cy.get(el.labelUnidadeInterna).should('contain', 'Unidade Interna')}
    validarTabelaUnidadeInterna() {cy.get(el.tabelaUnidadeInterna).should('contain', 'Unidade Interna')}
    validarLabelUnidadeExterna() {cy.get(el.labelUnidadeExterna).should('contain', 'Unidade Externa')}
    validarTabelaUnidadeExterna() {cy.get(el.tabelaUnidadeExterna).should('contain', 'Unidade Externa')}

    //validar campos de texto de Dilações de Prazos
    validarLabelSeiRequerimento() {cy.get(el.labelSeiRequerimento).should('contain', 'SEI do Requerimento')}
    validarLabelSeiDeferimentoIndeferimento() {cy.get(el.labelSeiDeferimentoIndeferimento).should('contain', 'SEI do Deferimento/ Indeferimento')}
    validarLabelStatusModal() {cy.get(el.labelStatusModal).should('contain', 'Status')}
    validarLabelDataLimiteDilacaoPrazo() {cy.get(el.labelDataLimiteDilacaoPrazo).should('contain', 'Data Limite Após Dilação de Prazo')}

    //validando campos de texto de relatório de acompanhamento de exigências
    validarLabelRaEmpreendimentoDemanda() {cy.get(el.labelRaEmpreendimentoDemanda).should('contain', 'RA / EMPREENDIMENTO / DEMANDA')}
    validarLabelNumeroModal() {cy.get(el.labelNumeroModal).should('contain', 'Nº')}
    validarLabelUnidadesResponsaveis() {cy.get(el.labelUnidadesResponsaveis).should('contain', 'Unidades Responsáveis')}
    validarLabelDataLimiteModal() {cy.get(el.labelDataLimiteModal).should('contain', 'Data Limite')}
    validarLabelInformativa() {cy.get(el.labelInformativa).should('contain', 'Informativa')}
    validarLabelDispensada() {cy.get(el.labelDispensada).should('contain', 'Dispensada')}
    validarLabelSeiDilacaoAnalise() {cy.get(el.labelSeiDilacaoAnalise).should('contain', 'SEI da Dilação em Analise')}
    validarLabelDescricaoProvidenciaModal() {cy.get(el.labelDescricaoProvidenciaModal).should('contain', 'Descrição Providência')}
    validarLabelDescricaoExigenciaModal() {cy.get(el.labelDescricaoExigenciaModal).should('contain', 'Descrição Exigência')}
    validarLabelCumprida() {cy.get(el.labelCumprida).should('contain', 'Cumprida')}
    validarLabelEmExecucao() {cy.get(el.labelEmExecucao).should('contain', 'Em Execução')}
    validarLabelTemaModal() {cy.get(el.labelTemaModal).should('contain', 'Tema')}
    validarLabelEstudosServRelacionados() {cy.get(el.labelEstudosServRelacionados).should('contain', 'Estudos/Serv Relacionados')}
    validarLabelPendente() {cy.get(el.labelPendente).should('contain', 'Pendente')}
